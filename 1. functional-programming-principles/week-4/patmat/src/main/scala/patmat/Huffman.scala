...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    @tailrec
    def mapAcc(chars: List[Char], acc: Map[Char, Int]): Map[Char, Int] = {
      chars match {
        case List() => acc
        case char :: rest =>
          mapAcc(rest, acc + (char -> (acc(char) + 1)))
      }
    }

    mapAcc(chars, Map.empty[Char, Int] withDefaultValue 0).toList
  }
  ...
  /** helper function */
  def insertLeaf(x: Leaf, xs: List[Leaf]): List[Leaf] = xs match {
    case List() => List(x)
    case y :: ys => if (x.weight <= y.weight) x :: xs else y :: insertLeaf(x, ys)
  }
  ...
  /** helper function */
  def xSmallerThanY(x: CodeTree, y: CodeTree): Boolean = x match {
  ...
  def insertCodeTree(x: CodeTree, xs: List[CodeTree]): List[CodeTree] = xs match {
    case List() => List(x)
    case y :: ys => if (xSmallerThanY(x, y)) x :: xs else y :: insertCodeTree(x, ys)
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def quickEncodeAcc(codeTable: CodeTable, text: List[Char], acc: List[Bit]): List[Bit] =
      if (text.isEmpty) acc
      else quickEncodeAcc(codeTable, text.tail, acc ::: codeBits(codeTable)(text.head))
