...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      initial.head #:: from(initial.tail++more, explored ++ more.map { case (b, _) => b }.to(Set))
    }
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(LazyList((Block(startPos, startPos), List())), Set(startBlock))
